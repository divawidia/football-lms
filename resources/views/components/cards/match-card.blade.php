    ...
    <div class="card-body">
        <div class="row">
                ...
                <h2 class="mb-0">
                    @if($latestMatch == true)
                        @if($match->homeTeam != null)
                            @php($homeTeam = $match->teams()->where('teamId', $match->homeTeamId)->first())
                            {{ $homeTeam->pivot->teamScore }}
                        @endif
                        -
                        @if($match->matchType == 'Internal Match' && $match->awayTeam != null)
                            @php($awayTeam = $match->teams()->where('teamId', $match->awayTeamId)->first())
                            {{ $awayTeam->pivot->teamScore }}
                        @elseif($match->externalTeam != null)
            ...
            <div class="col-4 d-flex flex-column-reverse flex-md-row align-items-center justify-content-end">
                    @if($match->matchType == 'Internal Match' && $match->awayTeam)
