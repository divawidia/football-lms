    ...
    <div class="container page__container d-flex flex-column pt-32pt">
        ...
        <ol class="breadcrumb p-0 m-0">
            ...
            <li class="breadcrumb-item"><a
                        href="{{ route('match-schedules.show', $data->id) }}">Match {{ $data->teams[0]->teamName }}
                    Vs. {{ $data->teams[1]->teamName }}</a></li>
                                        ...
                                        <select class="form-control form-select @error('competitionId') is-invalid @enderror"
                                            ...
                                            @foreach($competitions AS $competition)
                                                <option value="{{ $competition->id }}"
                                                        @selected(old('competitionId', $data->competitionId) == $competition->id) data-avatar-src="{{ Storage::url($competition->logo) }}">{{ $competition->name }}</option>
                                            @endforeach
                                        </select>
                                    @endif
                                    @error('competitionId')
